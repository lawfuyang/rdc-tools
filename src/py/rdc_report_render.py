...
def report_caveats() -> List[str]:
    ...
    return [
        ...
        'The one severity-table recipe that names an offline command is `aliased-write`\'s (`memory`): the '
        'overlapping placements it is about are a section of that command\'s output, and no driver command '
        'prints them. A `read-before-write` finding carries the chunk stream\'s own verdict with it, one line '
        'per resource in its evidence: the writer chain behind the flagged read, which is what tells a static '
        'asset (no writer in this frame at all) from an ordering bug (the write comes later) -- the numbering '
        'on that line is the file\'s chunk index, not the finding\'s eids (REFERENCE §4.15). '
