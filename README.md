...
| `provenance` | the chain of writers behind one use of a resource, back through copies and resolves, ending at the clear, the call that wrote it, a discard, the resource's creation, or "no writer in this frame" — the answer to "why is this buffer zero" behind the `read-before-write` flag (`provenance <rdc> <resId> [chunkIndex]`, exit 1 when there is no such use) | `provenance 'capture.rdc' res2263 15601` |
| `memory` | what the frame's memory adds up to: placement and kind with byte totals, capture-relative lifetimes, the aliasing barriers memory is handed over with, the placed pairs whose byte ranges overlap while both are live (and among them the ones written while the other side still reads, with whether an aliasing barrier declares the handover), heaps ranked by size, never-read bytes | `memory 'capture.rdc' 40` |
| `hazards` | the conflicts a frame commits against its own resource states and bindings, offline: a use whose resource the frame leaves in a state that forbids it (a read in `CopyDest`, a depth target in `DepthRead`), and one event using a resource two ways at once (a target also read through the signature, an SRV that is also a UAV); rows fold per distinct conflict with their count, and `--format csv` or `markdown` flattens them (exit 1 on a `certain` finding) | `hazards 'capture.rdc' 20` |
| `samplers` | every sampler the frame creates — a signature's static array decoded and each written heap slot — with filter, address mode, LOD range and comparison, plus the (sampler, texture) pairs a table or a register space binds together, each checked against the texture's mip count (exit 1 on a `certain` verdict) | `samplers 'capture.rdc' 20 --eid 2731` |
