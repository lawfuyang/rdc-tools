...
with a file *chunk index* -- a mapping no file read produces, and none this tool builds -- so a slot is
...
def bundle_findings(check: SigCheck, bundle_dir: str) -> Tuple[List[SigFinding], List[str]]:
    ...
    # an event -- no event-to-chunk mapping exists in this tool -- so the two sets cover
