...
with a file *chunk index* -- a mapping no file read produces (`Upstream`, ROADMAP §2) -- so a slot is
...
def bundle_findings(check: SigCheck, bundle_dir: str) -> Tuple[List[SigFinding], List[str]]:
    ...
    # an event -- no event-to-chunk mapping exists (`Upstream`, ROADMAP §2) -- so the two sets cover
