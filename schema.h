...
//: they all declare. Split out of replay_dump.cpp because it is *data* -- the biggest single block in
//: that file -- while the logic that prints, writes and checks it stays with the tool.
//
...
//: Stamped into every document, so a consumer can refuse a shape it does not understand. `constexpr`
//: rather than an extern: an int, one copy per translation unit, no linkage puzzle.
